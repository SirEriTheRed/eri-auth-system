...
- [ ] **H3** `src/routes/v1/auth/refresh.ts:28-33` + `login.ts:84-89` — CSRF-vulnerable: `/refresh` is GET (fetchable cross-origin via `<img>`/`<script>`), cookie is `sameSite: 'none'`, no CSRF token or Origin check. Change to POST, add Origin validation.
