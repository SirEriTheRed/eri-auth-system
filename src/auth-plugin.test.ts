...
describe('PATCH /v1/auth/logout', () => {
  ...
  it('returns 200 and revokes the token on a valid refresh cookie', async () => {
    ...
    expect(response.statusCode).toBe(200);
    // NOTE: source code reads `request.cookies.refeshToken` (typo), so `revokeToken`
    // is called with `undefined` instead of the actual token value.
    expect(mocks.revokeToken).toHaveBeenCalledWith(undefined);
  });
